...
@api_view(["GET"])
def getEncKeys(request, target_ID):
    # Simulated KM behavior for getting encryption keys
    # In a real QKD KM, this would trigger key generation or fetch from buffer
    key_id = str(uuid.uuid4())
    # Generate a random 256-bit key for simulation (AES-256 or OTP seed)
    key_value = base64.b64encode(os.urandom(32)).decode('utf-8')
    
    # Store the key so the receiver can fetch it
    # Note: In real QKD, the key would already be at the receiver's KM
    QuantumKey.objects.create(
        key_id=key_id,
        key_value=key_value,
        source_sae=request.GET.get('source_ID', 'unknown'),
        target_sae=target_ID
    )
...
@api_view(["GET"])
def getDecKeys(request, source_ID):
    key_ID = request.GET.get('key_ID')
    if not key_ID:
        return Response({"Message": "key_ID required", "Status": "Negative"}, status=400)
    
    try:
        q_key = QuantumKey.objects.get(key_id=key_ID)
        # In a real simulation, we'd verify source_ID matches target_sae etc.
        return Response({
            "keys": [
                {
                    "key_ID": q_key.key_id,
    ...
    except QuantumKey.DoesNotExist:
        return Response({"Message": "Key not found", "Status": "Negative"}, status=404)
